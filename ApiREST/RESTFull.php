<?php  
 ...
 require_once('../Models/ussers.php');

 class RESTFull extends Rest 
 {  
	private $_metodo;  
	private $_argumentos; 
	...
	{  
		...
		//Array ( [0] => [1] => [2] => [3] => [4] => controller [5] => [6] => [7] => method [8] => [9] => [10] => [11] => args [12] => [13] => [14] => )
			...
			if (method_exists($this, $this->_metodo)) 
			{  
	...
	{  
		...
		if (isset($this->datosPeticion['signin_username'], $this->datosPeticion['signin_password'])) 
		{  
			$datosUsuario = array(); 
			$datosUsuario['nickname'] = $this->datosPeticion['signin_username'];  
			$datosUsuario['password'] = $this->datosPeticion['signin_password']; 

			if (!empty($datosUsuario['nickname']) && !empty($datosUsuario['password'])) 
			{  
				$usuario = new Ussers();
				$usuario->get($datosUsuario);
